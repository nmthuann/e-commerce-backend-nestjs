...
@Injectable()
export class OrderDetailService extends AbstractBaseService<OrderDetailEntity> implements IOrderDetailService {
  constructor(
      ...
      for (const orderDetail of orderDetails) {
        ...
        if (product) {
          ...
          if (findDiscount.discount) {
            totalPrice += quantity * (product.price - product.price * (findDiscount.discount.percent / 100))
