...
export abstract class AbstractBaseService<T> implements IBaseService<T> {
  ...
  async getAll(): Promise<T[]> {
    try {
      ...
      return await this.baseRepository.createQueryBuilder(tableName).getMany()
    } catch (error) {
  ...
  async getOneById(id: number | string | ObjectId): Promise<T | null> {
    try {
      const tableName = this.baseRepository.metadata.tableName
      return await this.baseRepository.createQueryBuilder(tableName).where(`${tableName}.id = :id`, { id }).getOne()
    } catch (error) {
  ...
  async deleteOneById(id: number): Promise<void> {
    try {
      await this.baseRepository.createQueryBuilder().softDelete().where('id = :id', { id }).execute()
    } catch (error) {
