...
import { IEmployeeService } from 'src/modules/v1/modules/users/employee/Employee.service.interface'
...
import { IDiscountService } from '../../products/discount/discount.service.interface'
...
@Injectable()
export class OrderService extends AbstractBaseService<OrderEntity> implements IOrderService {
  constructor(
  ...
  async createNewOrderOffline(data: OrderOfflineDto): Promise<OrderEntity> {
    ...
    try {
      ...
      const getTotalPriceInOrderDetail = await this.orderDetailService.getTotalPriceByOrderId(orderCreated.order_id)
      ...
      const findPayment: PaymentEntity = await this.paymnetService.getOneById(1)
  ...
  async getTaskOrders(): Promise<GetTaskOrdersDto[]> {
    ...
    for (const order of findOrders) {
      ...
      const customer =
        (await Promise.resolve(order.user)).last_name + ' ' + (await Promise.resolve(order.user)).first_name //
      ...
      taskOrder.employee =
        order.status === 'pending' ? 'Not Employee' : (await Promise.resolve(order.employee)).employee_id
  ...
  async getCountProductSold(): Promise<number> {
    ...
    let countProduct = 0
    for (const order of findCompletedOrders) {
      countProduct = countProduct + (await this.orderDetailService.countProductSold(order.order_id))
    }
    return countProduct
  }
